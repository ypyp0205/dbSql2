--제약조건 활성화 / 비활성화
--ARTER TABLE 테이블명 ENABLE OR DISABLE CONSTRAINT 제약조건명;
...
--이미 위에서 실행한 두개의 INSERT 구문에 의해 같은 부서번호를 갖는 테이터가
...
SELECT *
FROM user_constraints
WHERE table_name = 'BUYER';   --제약조건 이름은 여기

SELECT *
FROM user_cons_columns              --제약조건 컬럼은 여기
...
COMMENT ON TABLE dept IS '부서';  -------------테이블 주석추가하는법임

--컬럼 주석
--COMMENT ON COLUMN 테이블명.컬럼명 IS '주석';0
...
UPDATE emp SET deptno = 30                       -----------------수정하는법
WHERE ename = 'KING';
...
--CREATE DEQUENCE 시퀀스명
--[옵션....]

CREATE SEQUENCE seq_board;

--시퀀스 사용방법 : 시쿼스명.nextval
...
--FOERIGN KEY 제약 추가 : fk_dept_deptno
...
--같이저장되어 있는ㄴ rowid 값을 통해 table에 접근을 한다.
